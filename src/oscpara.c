 ...
 * @return
 */
int _simulate_damped_os_parallel_mpi_omp(double max_amplitude, double length, double mass, double gravity, double k,
                                         double Ao,
                                         double Vo, double FI,
                                         ...
                                         int number_of_files)
{
    ...
    sprintf(_file_name, "%d_damped_os_parallel_v1_displacement_%d-%02d-%02d %02d:%02d:%02d.txt", world_rank,
                                           ...
                                           int number_of_files)
{
    ...
    if(world_rank==0)
    {
        ...
        printf("Started Job 1 at: %d-%02d-%02d %02d:%02d:%02d Processor: %s Rank: %d.\n", tm.tm_year + 1900,
               tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, processor_name, world_rank);
        double Wo=sqrt(k/mass);
        W=sqrt(Wo - pow(damping_coefficent/2*mass, 2));
        coefficient_calc=-damping_coefficent/(2*mass);
        RESULTS[0]=max_amplitude;
        ...
        printf("\nEnded Job 1 at: %d-%02d-%02d %02d:%02d:%02d Processor: %s Rank: %d Execution Time: %f sec.\n",
               tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, processor_name,
                                 ...
                                 double time_limit, double step_size, double damping_coefficent, int number_of_files)
{
    ...
    time_t tim=time(NULL);
    struct tm tm=*localtime(&tim);
    sprintf(_file_name, "%damped_os_parallel_v2_displacement%d-%02d-%02d %02d:%02d:%02d.txt", world_rank,
            tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
    p_dis=fopen(_file_name, "w");
 ...
 * @return
 */
int _execution_time_damped_os_parallel_mpi(double max_amplitude, double length, double mass, double gravity, double k,
                                           double Ao,
                                           double Vo, double FI,
                                           double time_limit, double step_size, double damping_coefficent,
                                           int number_of_files)
...
#pragma omp parallel for num_threads(number_of_threads) private(CALCULATIONS, RESULTS)
    ...
    printf("\nEnded Calculation at: %d-%02d-%02d %02d:%02d:%02d Execution Time: %f sec.\n", tm.tm_year + 1900,
           tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, end_time - start_time);
    tim=time(NULL);
    tm=*localtime(&tim);
