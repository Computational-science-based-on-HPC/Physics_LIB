    ...
    else
    {
        ...
        for (; i < endIndex; ++i)
        {
            for (ll x = 0; x < numSpacePoint; ++x)
            {
                if (my_rank == 1)
                {
                    fprintf(fptr1, "%f ", _get_value_1D_mpi(&time_param, &space_param, x, i, precision));
                }
                else if (my_rank == 2)
                {
                    fprintf(fptr2, "%f ", _get_value_1D_mpi(&time_param, &space_param, x, i, precision));
                }
                else if (my_rank == 3)
                {
                    fprintf(fptr3, "%f ", _get_value_1D_mpi(&time_param, &space_param, x, i, precision));
                }
            }
            if (my_rank == 1)
            {
